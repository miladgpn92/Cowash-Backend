...
namespace Web.Pages.DTCMS.Article
{
    ...
    {
        private readonly IRepository<Entities.Article> _repository;
        private readonly ISlugService<Entities.Article> _slugService;

        public UpsertModel(IRepository<Entities.Article> repository, ISlugService<Entities.Article> slugService, IMapper mapper)
        {
        ...
        [BindProperty]
        public ArticleDto? Items { get; set; } = new();
        ...
        public async Task OnGetAsync(int? id, bool? IsReturn, CancellationToken cancellationToken)
        {
            ...
            if (id.HasValue)
            {
                ...
                Items = await _repository.TableNoTracking.ProjectTo<ArticleDto>(Mapper.ConfigurationProvider)
        ...
        public async Task<IActionResult> OnPostAsync(CancellationToken cancellationToken)
        {
            ...
            if (IsEdit)
            {
                ...
                {
                    return Redirect($@"/article/{HttpUtility.UrlEncode(model.Slug)}");
                }
            }
            else
            {
