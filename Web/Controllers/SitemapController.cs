...
using Services.Services.CMS.PortfolioCategory;
    ...
    public class SitemapController : Controller
    {
        ...
        public async Task<IActionResult> SitemapAsync(CancellationToken cancellationToken)
        {
            ...
            await ProcessItemsAsync(
                item => BuildUrl(baseUrl, $"/article/{item.Slug}"),
                _articleService.List(1, 1000, null, null, null, cancellationToken),
                BuildUrl(baseUrl, "/articles")
            );

            await ProcessItemsAsync(
                item => BuildUrl(baseUrl, $"/articles/{item.Slug}"),
                _articleCategoryService.List(1, 1000, null, null, cancellationToken)
            );

            await ProcessItemsAsync(
                item => BuildUrl(baseUrl, $"/p/{item.Slug}"),
                _dynamicPageService.List(1, 1000, null, null, cancellationToken),
                BuildUrl(baseUrl, "/pages")
            );
